...
class Program
{
    ...
    static void Main()
    {
        ...
        var debt2 = debtors.Where(debtor => debtor.Email.EndsWith("rhyta.com") || debtor.Email.EndsWith("dayrep.com"));
        foreach (var debtor in debt2)
        {
            Console.WriteLine(debtor);
        }

        // 3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
        var debt3 = debtors.Where(debtor => debtor.BirthDay.Year >= DateTime.Now.Year - 36 && debtor.BirthDay.Year <= DateTime.Now.Year - 26);
        foreach (var debtor in debt3)
        {
            Console.WriteLine(debtor);
        }

        // 4) Borcu 5000 - den cox olmayan borclularic cixartmag
        var debt4 = debtors.Where(debtor => debtor.Debt < 5000);
        foreach (var debtor in debt4)
        {
            Console.WriteLine(debtor);
        }

        // 5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
        var debt5 = debtors.Where(debtor => debtor.FullName.Length > 18 && debtor.Phone.Count(n => n == '7') >= 2);
        foreach (var debtor in debt5)
        {
            Console.WriteLine(debtor);
        }

        // 7) Qishda anadan olan borclulari cixardmaq
        var debt7 = debtors.Where(debtor => debtor.BirthDay.Month != 12 && debtor.BirthDay.Month != 1 && debtor.BirthDay.Month != 2);
        foreach (var debtor in debt7)
        {
            Console.WriteLine(debtor);
        }
        ...
        var Debtors8 = debtors.Where(debtor => debtor.Debt > debtors.Average(d => d.Debt)).OrderBy(debtor => debtor.FullName);
        ...
        var debt9 = debtors.Where(debtor => !debtor.Phone.Contains('8')).Select(debtor => new { debtor.FullName, debtor.BirthDay, debtor.Debt });
        foreach (var debtor in debt9)
        {
            Console.WriteLine(debtor);
        }
        ...
        var debt11 = debtors.Where(debtor => debtor.FullName.Distinct().Count() == 3).OrderBy(debtor => debtor.FullName);
        foreach (var debtor in debt11)
        {
            Console.WriteLine(debtor);
        }

        // 13) Borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
        var birthYears = debtors.Select(debtor => debtor.BirthDay.Year);
        var mostCommonBirthYear = birthYears.GroupBy(year => year).OrderByDescending(group => group.Count()).First().Key;
        var debt13 = debtors.Where(debtor => debtor.BirthDay.Year == mostCommonBirthYear);
        foreach (var debtor in debt13)
        {
            Console.WriteLine(debtor);
        }

        // 14) Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
        var debt14 = debtors.OrderByDescending(debtor => debtor.Debt).Take(5);
        foreach (var debtor in debt14)
        {
            Console.WriteLine(debtor);
        }
        ...
        var debt16 = debtors.Where(debtor => debtor.BirthDay.Year < 1945 && debtor.BirthDay.Year > 1939);
        foreach (var debtor in debt16)
        {
            Console.WriteLine(debtor);
        }

        // 18) Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
        var debt18 = debtors.Where(debtor => debtor.Phone.Distinct().Count() == debtor.Phone.Length).Select(debtor => new { debtor.FullName, debtor.Debt });
        foreach (var debtor in debt18)
        {
            Console.WriteLine(debtor);
        }
        ...
        var debt19 = debtors.Where(debtor => (debtors.Sum(d => d.Debt) - debtor.Debt) <= ((today.AddYears(1) - debtor.BirthDay).TotalDays / 365) * 500);
        foreach (var debtor in debt19)
        {
            Console.WriteLine(debtor);
        }

        // 20) Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq yazilmayanlari yaz
        var debt20 = debtors.Where(debtor => debtor.FullName.ToLower().Distinct().Intersect("smile").Count() == 4);
        foreach (var debtor in debt20)
        {
            Console.WriteLine(debtor);
        }
    }
